...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!user.companyId) {
      return NextResponse.json({ error: "No company account. Create one first." }, { status: 400 });
    }
    ...
    if (existingMember) {
      if (existingMember.is_active) {
        ...
        return NextResponse.json({
          error: "This person was previously deactivated. Reactivate them from the Team page instead.",
        }, { status: 409 });
      }
    }
    ...
    if (existingOtherCompany) {
      return NextResponse.json({
        error: "This email is already associated with another company on Hirasys.",
      }, { status: 409 });
    }
    ...
    const invitation = await queryOne(
      `INSERT INTO invitations (company_id, email, role, invited_by, token)
       VALUES ($1, $2, $3, $4, $5)
       RETURNING *`,
      [user.companyId, email.trim().toLowerCase(), inviteRole, user.id, token]
    );
    ...
    try {
      ...
      console.log(`[Invite] Email sent to ${email} for ${company?.name}`);
    } catch (emailErr) {
    ...
    try {
      await query(
        ...
        [user.id, `Invitation sent to ${email} as ${inviteRole}`]
      );
    } catch {}
...
export async function DELETE(req: NextRequest) {
  try {
    ...
    if (invitation.status !== "PENDING") {
      return NextResponse.json({ error: `Cannot revoke — invitation is already ${invitation.status}` }, { status: 400 });
    }
